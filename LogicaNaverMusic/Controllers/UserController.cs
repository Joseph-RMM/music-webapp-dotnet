...
namespace LogicaNaverMusic.Controllers
{
    public class UserController
    {
        ...
        public bool VerifyFavoriteTrack(int idUser, int idTrack)
        {
            bool exist = false;

            var lst = (from u in modelDb.FavCancion
                      where u.idUser == idUser && u.idCancion == idTrack
                      select u).FirstOrDefault<FavCancion>();
        ...
        public bool VerifyFavoriteArtist(int idUser, int idArtist)
        {
            bool exist = false;

            var lst = (from u in modelDb.FavArtista
                       where u.idUser == idUser && u.idArtista == idArtist
                       select u).FirstOrDefault<FavArtista>();
